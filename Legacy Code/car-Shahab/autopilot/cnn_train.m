...
for epoch=start+1:opts.numEpochs
    ...
    if numel(params.gpus) <= 1
        ...
        if ~evaluateMode
            ...
            if epoch > 1
                va = [stats.val.objective];
                tr = [stats.train.objective];
                nva = stats.val(end).num;
                ntr = stats.train(end).num;
%                 va = va * nva;
%                 tr = tr * ntr;
                old_overall_error = tr;
                this_overall_error = state.stats.train.objective;
                old_val_error = va;
                this_val_error = state.stats.val.objective;
                fprintf('%d\tERROR: train %.3f (best %.3f)\tval %.3f (best %.3f)', epoch, ...
                ...
                if numel(va) > 100
                    ...
                    fprintf('\t(avg: %.3f %.3f)', avg_tr, avg_va);
                end
                fprintf('\n');
