...
%     TRAJ_CPU = zeros(9, T, K);
    ...
    splined_y = zeros(1, T);
    for i = 1:T
        splined_x(i) = splined_history(1,1) * i^2 + splined_history(1,2) * i + splined_history(1,3);
        splined_y(i) = splined_history(2,1) * i^2 + splined_history(2,2) * i + splined_history(2,3);
    end
...
splined_history = zeros(3, iter_dimension);
for i = 1:iter_dimension
    if count*dt == 0 && history(i, 1) == history(i, 2)
        %change the splines to polyfit function
        cc = polyfit([count*dt, 50], history(i,[2 3]), 2);
    elseif count*dt == 50 && history(i, 3) == history(i, 2)
        cc = polyfit([0, count*dt], history(i,[1 2]), 2);
    else
        cc = polyfit([0, count*dt, 50], history(i,:), 2);
    end
    spline_size = size(cc);
    for j = 1:spline_size(2)
        splined_history(j, i) = cc(1, j);
    end
end
