...
%--------------------------------------------------------------------------
    ...
    methods (Access = public)
        ...
        function InsertKeyBeta(this,key)
            %------------------------ InsertKeyBeta ---------------------------
            % Syntax:       H.InsertKeyBeta(key);
            %               
            % Inputs:       key is a node
            %               
            % Description:  Inserts key into H and checks for existing key
            %--------------------------------------------------------------
            
            for i = 1:this.k
                %In node the '==' function is defined by equal x and y
                %values in both nodes
                if this.x(i) == key
                    if this.x(i).g < key.g
                    this.x(i) = key;
                    end
