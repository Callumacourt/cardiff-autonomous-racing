...
for epoch=start+1:opts.numEpochs
    ...
    if numel(params.gpus) <= 1
        ...
        if ~evaluateMode
            ...
            if exist(modelPath(epoch - 2), 'file')
                delete(modelPath(epoch - 2));
            end
            if epoch > 1
                va = [stats.val.top1err];
                tr = [stats.train.top1err];
                nva = stats.val(end).num;
                ntr = stats.train(end).num;
                va = va * nva;
                tr = tr * ntr;
                old_overall_error = va + tr;
                this_overall_error = state.stats.val.top1err * state.stats.val.num + ...
                    state.stats.train.top1err * state.stats.train.num;
                old_val_error = va;
                this_val_error = state.stats.val.top1err * state.stats.val.num;
                fprintf('%d\tERROR: overall %.0f (best %.0f)\tval %.0f (best %.0f)', epoch, ...
                    this_overall_error, min(old_overall_error), ...
                    this_val_error, min(old_val_error));
                if numel(va) > 100
                    avg_va = mean(va(end-99:end));
                    avg_tr = mean(tr(end-99:end));
                    fprintf('\t(avg: %.2f %.2f)', avg_tr, avg_va);
                end
                fprintf('\n');
                if this_overall_error <= min(old_overall_error)
                    fprintf('BEST OVERALL!\n');
                    saveState(fullfile(opts.expDir, 'best.mat'), net, state);
                end
                if this_val_error <= min(old_val_error)
                    fprintf('BEST VAL!\n');
                    saveState(fullfile(opts.expDir, 'best-val.mat'), net, state);
                end
            else
                saveState(fullfile(opts.expDir, 'best.mat'), net, state);
                saveState(fullfile(opts.expDir, 'best-val.mat'), net, state);
        ...
        spmd
            ...
            if labindex == 1 && ~evaluateMode
                saveState(modelPath(epoch), net, state) ;
                if exist(modelPath(epoch - 1), 'file')
...
%         if ~exist('ha', 'var')
%         ha = tight_subplot(1, numel(plots)-1, [.01 .03], [.1 .01], [.01 .01]);
%         end
        for p = plots(1:end-1) % KS
            p = char(p) ;
            ...
            %             subplot(1,numel(plots),find(strcmp(p,plots))) ; % KS
            subplot(1,numel(plots)-1,find(strcmp(p,plots))) ;
%             axes(ha(find(strcmp(p,plots))));
            %             plot(1:epoch, values','o-') ;
            show = 200;
            semilogy(max(1, epoch - show):epoch, values(:, max(1, epoch - show):epoch)','.:');
            
            % Filtered plot
            %             values_f = filtfilt(filt_b, filt_a, values')';
            wnd = 31;
            
            if size(values, 2) > wnd
                values_f = sgolayfilt(values', 1, wnd)';
                hold on
                plot(max(wnd, epoch - show):epoch, values_f(1, max(wnd, epoch - show):epoch), 'b', 'LineWidth', 2);
                plot(max(wnd, epoch - show):epoch, values_f(2, max(wnd, epoch - show):epoch), 'r', 'LineWidth', 2);
                hold off
            end
...
%             de = 100;
%             semilogy(1:(epoch + de), ...
%                 exp(polyval(polyfit(loge, logv1, 2), log(1:(epoch + de)))), 'b:');
%             semilogy(1:(epoch + de), ...
%                 exp(polyval(polyfit(loge, logv2, 2), log(1:(epoch + de)))), 'r:');
%             hold off
            
%             ylim = get(gca, 'YLim');
%             if strcmpi(p, 'top1err') || strcmpi(p, 'top5err')
%                 set(gca, 'Ylim', [0 1]);
%                 set(gca, 'YTick', [0.0:0.001:0.009 0.01:0.01:0.09 0.1:0.1:1]);
%             else
                %                 set(gca, 'YTick', round(ylim(1)*10:1:ylim(2)*10)/10);
%             end
            xlabel('epoch') ;
            title(p) ;
            legend(leg{:}) ;
        ...
        sc(w);
        %         switchFigure(3);
        %         w = net.layers{4}.weights{1};
        %         sc(reshape(w, size(w, 1), size(w, 2), 1, []));
        %         switchFigure(4);
...
for t=1:params.batchSize:numel(subset)
    ...
    for f = setdiff(fieldnames(stats)', {'num', 'time'})
        f = char(f) ;
        if strcmp(f, 'top1err') || strcmp(f, 'top5err')
            %             fprintf(' %s: %.3f%%', f, stats.(f) * 100) ;
        else
