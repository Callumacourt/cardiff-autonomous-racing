...
//#if defined(__GNUC__) && !defined(__BEOS__) && !defined(__FORCE_SSE2__)
#  define FPU_CONTROL
...
static inline void fpu_restore(fpu_control fpu){
    int16_t temp = fpu;
  __asm__ __volatile__("fldcw %0":: "m"(temp));
}

// assumes the FPU is in round mode!
static inline int ftoi(double f){  // yes, double!  Otherwise,                                             we get extra fst/fld to
//                                             truncate precision
  int i;
