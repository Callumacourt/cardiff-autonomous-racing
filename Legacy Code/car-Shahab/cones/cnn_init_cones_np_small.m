function net = cnn_init_cones_small(Nclasses, h, w, dim)
...
net.layers{end+1} = struct('type', 'conv', ...
    'weights', {{f * randn(7, 5, dim, 4, 'single'), zeros(1, 4, 'single')}}, ...
    'stride', 1, ...
    'pad', 0);
...
% BLOCK 2
net.layers{end+1} = struct('type', 'conv', ...
    'weights', {{f*randn(7, 5, 4, 4, 'single'),zeros(1, 4,'single')}}, ...
    'stride', 1, ...
    'pad', 0) ;
net = vl_simplenn_tidy(net); res = vl_simplenn(net, in);
fprintf('in: %d x %d x %d --> out: %d x %d x %d\n', ...
...
% BLOCK 4
net.layers{end+1} = struct('type', 'conv', ...
    'weights', {{f * randn(6, 6, 4, 8, 'single'), zeros(1, 8, 'single')}}, ...
    'stride', 1, ...
    'pad', 0) ;
net = vl_simplenn_tidy(net); res = vl_simplenn(net, in);
fprintf('in: %d x %d x %d --> out: %d x %d x %d\n', ...
...
% FC
net.layers{end+1} = struct('type', 'conv', ...
    'weights', {{f*randn(1, 1, 8, Nclasses, 'single'),  zeros(1, Nclasses, 'single')}}, ...
    'stride', 1, ...
    'pad', 0) ;
net = vl_simplenn_tidy(net); res = vl_simplenn(net, in);
fprintf('in: %d x %d x %d --> out: %d x %d x %d\n', ...
