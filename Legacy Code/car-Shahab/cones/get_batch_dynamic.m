function [images, labels] = get_batch_dynamic(imdb, batch)
...
idx_bg = find(imdb.images.label == 3 & imdb.images.set == 1);
images(:, :, :, Nbg_dynamic+1:Nbg_dynamic+Nbg_static) = ...
    imdb.images.data(:, :, :, idx_bg(randi(numel(idx_bg), [1 Nbg_static])));
labels(:, :, :, Nbg_dynamic+1:Nbg_dynamic+Nbg_static) = 3;

Nfg = N - Nbg_dynamic - Nbg_static;

correct_labels = imdb.images.label == 1 | imdb.images.label == 2 | imdb.images.label == 4;
...
for i = 1:size(im_fg, 4)
    if rand() > 0.75
        ...
        labels_fg(i) = 3; % Becomes background
    end
end
