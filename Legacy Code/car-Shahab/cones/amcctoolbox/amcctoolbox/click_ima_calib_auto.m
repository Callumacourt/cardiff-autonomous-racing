...
try
    ...
    fprintf('Failed due to exception.\n');
    return;
end
...
if autocrnrst.fail==true
    ...
    eval(['n_sq_y_' num2str(kk) ' = NaN;']);
    eval(['extfail_' num2str(kk) ' = true;']);
    fprintf('Failed to find any corners.\n');
    return;
end

% If the figure for displaying doesn't exist, create it
checkerboard_display = figure(1);
% Put the image on screen
image(I); 
colormap(map); hold on;
% Place the image number as the title
...
% Check to see if a calibration board of the right size was found
if (size(grid_pts_mat,1) == ny_crnrs || size(grid_pts_mat,2) == nx_crnrs...
    && nx_crnrs*ny_crnrs == size(grid_pts_mat,1)*size(grid_pts_mat,2))

    fprintf('Checkboard transposed. Correcting...\n');
    % Need to invert some values so that they are the correct checkerboard
    % values
    
    grid_pts_matt = grid_pts_mat;
    clear grid_pts_mat;
    grid_pts_mat(:,:,1) = grid_pts_matt(:,:,1)';
    grid_pts_mat(:,:,2) = grid_pts_matt(:,:,2)';
    
end


if (size(grid_pts_mat,1) ~= nx_crnrs || size(grid_pts_mat,2) ~= ny_crnrs)
    % Not enough corners were found
    ...
    eval(['n_sq_y_' num2str(kk) ' = NaN;']);
    eval(['extfail_' num2str(kk) ' = true;']);
    fprintf('Failed to find enough corners. ');
    fprintf(['Found ' num2str(size(grid_pts_mat,1)) ' corners for X (expected ' num2str(nx_crnrs) '), ']);
    fprintf(['and ' num2str(size(grid_pts_mat,2)) ' corners for Y (expected ' num2str(ny_crnrs) ').\n']);
    return;
end
...
if sus
    ...
    fprintf('Success! Found the correct number of corners.\n');
end
