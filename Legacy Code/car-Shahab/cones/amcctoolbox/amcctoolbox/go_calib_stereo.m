...
if ~recompute_intrinsic_right,
    fprintf(1,'\nNo recomputation of the intrinsic parameters of the right camera (recompute_intrinsic_right = 0)\n');
    ...
    for kk = 1:n_ima,
        ...
        if active_images(kk),
            ...
            if ~est_aspect_ratio_left,
                if fisheye
                    [xl,dxldomckk,dxldTckk,dxldfl,dxldcl,dxldkl,dxldalphal] = project_points_fisheye(Xckk,omckk,Tckk,fc_left(1),cc_left,kc_left,alpha_c_left);
                    dxldkl(:,5) = ones(size(dxldkl(1),1));
                else
                ...
                if fisheye
                    [xl,dxldomckk,dxldTckk,dxldfl,dxldcl,dxldkl,dxldalphal] = project_points_fisheye(Xckk,omckk,Tckk,fc_left,cc_left,kc_left,alpha_c_left);
                    dxldkl(:,5) = ones(size(dxldkl(1),1));
                else
            ...
            if ~est_aspect_ratio_right,
                if fisheye
                    [xr,dxrdomr,dxrdTr,dxrdfr,dxrdcr,dxrdkr,dxrdalphar] = project_points_fisheye(Xckk,omr,Tr,fc_right(1),cc_right,kc_right,alpha_c_right);
                    dxrdkr(:,5) = ones(size(dxrdkr(1),1));
                else
                ...
                if fisheye
                    [xr,dxrdomr,dxrdTr,dxrdfr,dxrdcr,dxrdkr,dxrdalphar] = project_points_fisheye(Xckk,omr,Tr,fc_right,cc_right,kc_right,alpha_c_right);
                    dxrdkr(:,5) = ones(size(dxrdkr(1),1));
                else
...
if fisheye
    kc_right_error = param_error(16:19);
    kc_right = kc_right(1:4);
else
...
if fisheye
    fprintf(1,'Fisheye Distortion:            kc_left = [ %3.5f   %3.5f   %3.5f   %3.5f ] ± [ %3.5f   %3.5f   %3.5f   %3.5f ]\n',[kc_left;kc_left_error]);   
else
...
if fisheye
    fprintf(1,'Fisheye Distortion:            kc_right = [ %3.5f   %3.5f   %3.5f   %3.5f ] ± [ %3.5f   %3.5f   %3.5f   %3.5f ]\n',[kc_right;kc_right_error]);   
else
