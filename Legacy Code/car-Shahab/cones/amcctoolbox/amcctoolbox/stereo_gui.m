...
cell_list{4,1} = {'Undistort the calibration images','undistort_stereo_pair;'};
cell_list{4,2} = {'View the Epipolar Geometry','view_epipolar_geometry;'};
cell_list{5,1} = {'Export calibration to XML/YAML','export_opencv_intrinsics_stereo;'};
cell_list{5,2} = {'Show the Calibration Variables','view_calibration_variables;'};
cell_list{6,1} = {'Rectify the calibration images','rectify_stereo_pair;'};
cell_list{6,2} = {'Exit',['disp(''Bye. To run again, type stereo_gui.''); close(' num2str(fig_number) ');']}; %{'Exit','calib_gui;'};


show_window(cell_list,fig_number,title_figure,300,25,1,'Arial',16);
