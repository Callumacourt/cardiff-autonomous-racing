...
class ImageList(wx.Panel):
    ...
    def scan_images(self):
        ...
        for fn in self.fns:
            ...
            for bb in bboxes:
                count[bb[4]] += 1
                total_count[bb[4]] += 1
            pos = self.list.InsertItem(self.list.GetItemCount(), fn[truncate:])
    ...
    def set_bbox_count(self, count, pos = None):
        ...
        if count[1] > 0:
            self.list.SetItem(pos, 1, str(count[1]))
        if count[2] > 0:
            self.list.SetItem(pos, 2, str(count[2]))
        if count[3] > 0:
            self.list.SetItem(pos, 3, str(count[3]))
        if count[4] > 0:
            self.list.SetItem(pos, 4, str(count[4]))
    ...
    def select_prev_image_page(self):
        idx = max(self.selected_idx() - 10, 1)
        self.list.Select(idx)
        self.list.EnsureVisible(idx)
